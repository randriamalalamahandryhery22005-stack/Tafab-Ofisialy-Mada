...
(() => {
  ...
  async function openPageDetail(id){
    ...
    openModal(`<div class="modal-box page-premium-modal page-detail" data-page-id="${esc(id)}">
      ...
      <div class="page-stats page-stats-three"><div><b>${postCount}</b><span>Publications</span></div><div><b>${esc(x.category||'Autre')}</b><span>Catégorie</span></div><div><b>${ownerMe?'Vous':'Page'}</b><span>Compte</span></div></div>
      <div class="page-follow-line"><span><b>${followerCount}</b> abonnés</span><span>Suivi : <b>${follow.data?'Oui':'Non'}</b></span></div>
