...
Deno.serve(async req => {
  try {
    ...
    const {data,error}=await admin.rpc('tafab_apply_papi_coin_payment',{
      ...
      p_notification_token:token,
      p_payment_status:status,
      p_payment_method:String(body.paymentMethod||'').trim(),
      p_amount_mga:Math.round(Number(body.amount||0)),
      p_fee_mga:Math.round(Number(body.fee||0)),
      p_merchant_payment_reference:String(body.merchantPaymentReference||'').trim(),
